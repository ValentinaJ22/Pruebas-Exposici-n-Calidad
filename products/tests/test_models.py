...
class ProductTests(TestCase):
    ...
    def test_negative_price_product(self):
        ...
        try:
            product = Product.objects.create(title="Maletin", description="Cuero", price= Decimal(-5.00), image=SimpleUploadedFile(name='test_image.jpg', content=b'', content_type='image/jpeg'))
            # Si se permite, fallamos explícitamente
            self.fail("El sistema permitió crear un producto con precio negativo, lo cual no debería ser posible.")
        except Exception as e:
            # Verificar que se lanza una excepción, como ValidationError
            self.assertIsInstance(e, ValidationError, "No se lanzó un ValidationError para un precio negativo.")
